...
// All amounts in cents (stored as numbers, not bigint)
export const PRIZE_TIERS: PrizeTier[] = [
  ...
  { name: '4th Prize', matchCount: 4, requiresBonus: false, multiplier: 0,    isFixed: true, fixedAmount: 10000 * 100 },
  { name: '5th Prize', matchCount: 3, requiresBonus: false, multiplier: 0,    isFixed: true, fixedAmount: 100 * 100 },
  { name: '6th Prize', matchCount: 2, requiresBonus: true,  multiplier: 0,    isFixed: true, fixedAmount: 10 * 100 },
]
  ...
  for (const tier of PRIZE_TIERS) {
    if (tier.matchCount === matches && (!tier.requiresBonus || bonusMatch)) {
      const amount = tier.isFixed
        ? (tier.fixedAmount ?? 0)
        : Math.floor(jackpotAmount * tier.multiplier)
      return { tier, amount }
    }
  }
