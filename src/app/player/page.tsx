...
import { formatCurrency, formatShortDate, serializeBigInt } from '@/lib/utils'
            ...
            <div className={`mb-6 p-5 rounded-2xl flex items-start gap-3 border ${
              ...
              <div className="flex-1">
                ...
                <p className="text-white/40 text-sm mt-0.5">
                  {user.verificationStatus === 'PENDING'
                    ? 'Your documents are being reviewed. You\'ll be notified within 1–2 business days.'
                    : user.verificationStatus === 'REJECTED'
                ...
                {(data.winners as any[]).map((w: any) => (
                  <div key={w.id} className="flex items-center justify-between p-3 rounded-xl bg-yellow-500/5 border border-yellow-500/10">
                    ...
                    <div className="text-right">
                      <p className="text-yellow-400 font-mono font-bold">{formatCurrency(BigInt(w.prizeAmount))}</p>
                  ...
                  {(data.transactions as any[]).map((tx: any) => (
                    <div key={tx.id} className="p-4 flex items-center gap-3">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {BigInt(tx.amount) > 0n && (
                          <p className="text-white/40 text-xs">{formatCurrency(BigInt(tx.amount))}</p>
                        )}
                      </div>
                    </div>
                  ))}
